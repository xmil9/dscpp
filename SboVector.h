...
{
   construct_from(other, other.m_data);
}
...
{
   construct_from(ilist, ilist.begin());
}
...
template <typename T, std::size_t N> 
void SboVector<T, N>::move_from(SboVector&& other)
{
   if (other.on_heap())
   {
      // Steal heap memory.
